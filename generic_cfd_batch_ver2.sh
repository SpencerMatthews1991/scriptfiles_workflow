...
# Fluent-specific settings (optional - leave empty to use case file settings)
FLUENT_SIMULATION_TYPE=""  # "steady", "transient", or "" (auto-detect from case)
FLUENT_NUM_STEPS=""        # Number of iterations/time-steps, or "" (use case settings)
...
run_solver_case() {
    ...
    echo "DEBUG: Starting run_solver_case for $case_dir"
    echo "DEBUG: job_dir = $job_dir"
    echo "DEBUG: solver = $solver"
    ...
    echo "DEBUG: Changing to $job_dir"
    cd "$job_dir" || {
    ...
    echo "DEBUG: Running solver case for $solver"
...
; Fluent will use the iteration count/time settings already configured
/solve/iterate
